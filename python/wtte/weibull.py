...
def continuous_loglik(t, a, b, u=1, equality=False):
    if equality:
        ...
        loglik = u*np.log(pdf(t, a, b))+(1-u)*np.log(1.0-cdf(t, a, b))
    else: 
        # commonly optimized over: proportional terms w.r.t alpha,beta
        loglik = u*loglik(hazard(t,a,b))-loglik(cumulative_hazard(t,a,b))
...
def discrete_loglik(t, a, b, u=1,equality=False):
    if equality:
        # With equality instead of proportionality. 
        loglik = u*np.log(pmf(t, a, b))+(1-u)*np.log(1.0-cdf(t+1.0, a, b))
    else: 
        # commonly optimized over: proportional terms w.r.t alpha,beta
